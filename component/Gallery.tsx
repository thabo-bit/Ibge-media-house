...
type Props = {
  /** All images to display across the two marquee rows */
  images: string[];
  /** Optional captions matched by index */
  captions?: string[];
  /** How many images go in the top row (defaults to half) */
  split?: number;
};

export default function Gallery({ images, captions, split }: Props) {
  const [hoveredSrc, setHoveredSrc] = useState<string | null>(null);
  const [isPaused, setIsPaused] = useState(false);
  const hoverTimerRef = useRef<ReturnType<typeof setTimeout> | null>(null);

  // Split into two rows — defaults to half-and-half
  const mid = split ?? Math.ceil(images.length / 2);
  const rowOne = images.slice(0, mid);
  const rowTwo = images.slice(mid);
...
function GalleryItem({
  ...
  return (
    <div className="relative flex-shrink-0 w-[180px] sm:w-[220px] lg:w-[240px] aspect-[3/4]">
      <div
